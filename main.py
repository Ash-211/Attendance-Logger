...
def load_known_faces():
    ...
    if not known_face_names:
        messagebox.showwarning("Warning", "No valid known faces loaded.")
        return False
    ...
    while True:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if True in matches:
                ...
                insert_attendance("Unknown", "Unknown")
...
def add_student_gui():
    ...
    students = get_all_students()
    if any(prn == s_prn for _, s_prn in students):
    ...
    with open(image_path, 'rb') as file:
        insert_image(image_filename, file.read())
...
def modify_student_gui():
    ...
    if new_name and new_prn and update_student(old_prn, new_name.strip(), new_prn.strip()):
...
def export_attendance_by_date():
    ...
    df = pd.DataFrame([{"Name": r[0], "Status": r[1], "Timestamp": r[2]} for r in records])
    file_path = filedialog.asksaveasfilename(defaultextension=".xlsx", filetypes=[("Excel files", "*.xlsx")], initialfile=f"attendance_{date_str}.xlsx")
...
tk.Button(frame, text="Add Student", command=add_student_gui, bg="#4CAF50", fg="white", font=("Helvetica", 12), relief=tk.FLAT).grid(row=3, column=0, pady=10, sticky="ew")
tk.Button(frame, text="Remove Student", command=remove_student_gui, bg="#f44336", fg="white", font=("Helvetica", 12), relief=tk.FLAT).grid(row=3, column=1, pady=10, sticky="ew")
tk.Button(frame, text="Modify Student", command=modify_student_gui, bg="#2196F3", fg="white", font=("Helvetica", 12), relief=tk.FLAT).grid(row=3, column=2, pady=10, sticky="ew")
tk.Button(root, text="Start Video Attendance", command=start_video_attendance, bg="#FF9800", fg="white", font=("Helvetica", 14), relief=tk.FLAT).grid(row=1, column=0, pady=15, sticky="ew", padx=20)
tk.Button(root, text="Export Attendance by Date", command=export_attendance_by_date, bg="#9C27B0", fg="white", font=("Helvetica", 14), relief=tk.FLAT).grid(row=2, column=0, pady=15, sticky="ew", padx=20)
